...
const Captionriding = () => {
  ...
  const { user, finalpickup, finaldrop, pickupname, droploc, droplocname, pickupppp, Price } = useContext(Usercontext);
  
  // Get values with proper fallback priority
  const pickup = finalpickup || pickupname || pickupppp || localStorage.getItem("finalpickup") || "Unknown pickup";
  const dropoff = finaldrop || droplocname || localStorage.getItem("finaldrop") || "Unknown drop-off";
  const price = Price || (localStorage.getItem("ridePrice") ? Number(localStorage.getItem("ridePrice")) : 38.75);
  const [data, setData] = useState({});
  // Debug: Log what values we're using
  useEffect(() => {
    console.log("==========================================");
    console.log("CAPTIONRIDING - Values Check:");
    console.log("finalpickup (context):", finalpickup);
    console.log("finaldrop (context):", finaldrop);
    console.log("Price (context):", Price);
    console.log("pickup (final):", pickup);
    console.log("dropoff (final):", dropoff);
    console.log("price (final):", price);
    console.log("==========================================");
  }, [finalpickup, finaldrop, Price, pickup, dropoff, price]);

  // Listen for ride accepted events once and update `data` with the ride payload.
  useEffect(() => {
    const handleRideAccepted = ({ ride, driverId, driverSocketId }) => {
  ...
  const rideDetails = {
    passenger: {
      name: data && data.passenger ? data.passenger : (user && user.firstname) || (caption && caption.firstname) || localStorage.getItem("passengerName") || "",
      rating: "4.5",
    ...
    trip: {
      pickup: (data && data.pickup) || pickup,
      dropoff: (data && (data.drop || data.dropoff)) || dropoff,
      distance: "5.2 mi",
      time: "18 min",
      fare: `$${price}`,
    },
    car: {
      model: "Toyota Camry",
      color: (caption && caption.vechicle && caption.vechicle.color) || "red",
      license: (caption && caption.vechicle && caption.vechicle.plate) || "ka05mn5623",
    },
  };
    ...
    <div className="w-full h-screen flex flex-col bg-gray-900">
      <div className="relative w-full h-80 rounded-xl overflow-hidden shadow-lg">
        <div className="absolute inset-0 bg-gray-800">
          <UberMap className="w-full h-full object-cover opacity-90" />
          ...
          <div className="p-6">
              ...
              <div className="mb-6">
                <div className="mb-4">
                  <p className="text-gray-900 font-medium mb-2">
                    Enter passenger's OTP to start ride
                  </p>
            ...
            <div className="mb-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-gray-900 font-medium truncate">
                    {rideDetails.car.model}
                  </p>
                  <p className="text-gray-500 text-sm truncate">
                    {rideDetails.car.color} • {rideDetails.car.license}
                  </p>
                </div>
              </div>
